...
lemma carrier.smul_mem (hm : 0 < m) (q : Spec.T (A⁰_ f_deg)) (c x : A) (hx : x ∈ carrier q) :
  c • x ∈ carrier q :=
have mem_supr : ∀ x, x ∈ supr 𝒜, from λ x, (is_internal 𝒜).supr_eq_top.symm ▸ submodule.mem_top,
begin
  haveI : Π (i : ℕ) (x : (𝒜 i)), decidable (x ≠ 0) := λ _ _, classical.dec _,
  refine submodule.supr_induction 𝒜 (mem_supr c) (λ n a ha, _) _ _,
  { intros i,
    by_cases ineq1 : n ≤ i,
    { have eq1 : (graded_algebra.proj 𝒜 i) (a * x) =
      ...
      { exact calc (proj 𝒜 i) (a * x)
              = proj 𝒜 i ∑ j in support 𝒜 x, (a * (proj 𝒜 j x))
              : begin
                conv_lhs { rw [← sum_support_decompose 𝒜 x] },
                simp_rw [proj_apply],
                rw [finset.mul_sum],
              end
          ... = ∑ j in support 𝒜 x, (proj 𝒜 i (a * (proj 𝒜 j x)))
              : by rw linear_map.map_sum
          ... = ∑ j in support 𝒜 x, (ite (j = i - n) (proj 𝒜 i (a * (proj 𝒜 j x))) 0)
              : begin
          ...
          ... = ite (i - n ∈ support 𝒜 x) (a * (proj 𝒜 (i - n)) x) 0 : by rw finset.sum_ite_eq', },
...
lemma forward_backward {f : A} {m : ℕ}
  (hm : 0 < m)
  (f_deg : f ∈ 𝒜 m)
  (x : Spec.T (A⁰_ f_deg)) :
  forward.to_fun 𝒜 f_deg (backward.to_fun f_deg hm x) = x :=
begin
ext z, split,
{ intros hz,
  change z ∈ (forward.to_fun _ f_deg (⟨⟨⟨backward.carrier.as_ideal hm x, _⟩, _, _⟩, _⟩)).1 at hz,
  unfold forward.to_fun at hz,
  dsimp only at hz,
  erw forward.carrier_eq_carrier' at hz,
  unfold forward.carrier' at hz,
  erw [←ideal.submodule_span_eq, finsupp.span_eq_range_total, set.mem_range] at hz,
  obtain ⟨c, eq1⟩ := hz,
  erw [finsupp.total_apply, finsupp.sum] at eq1,
  erw ←eq1,
  ...
  { intros hz,
    ...
    { convert hz using 1,
      ...
      exact hα, },
    erw hα,
    have mem1 : α ∈ backward.carrier x,
    { intros j,
      ...
      { simp only [ineq1, graded_algebra.proj_apply],
        ...
        refl, },
    { simp only [proj_apply, decompose_of_mem_ne 𝒜 α_mem (ne.symm ineq1), zero_pow hm, mk_zero],
      exact submodule.zero_mem _, }, },
...
section backward_forward

lemma backward_forward {f : A} {m : ℕ}
  (hm : 0 < m)
  (f_deg : f ∈ 𝒜 m)
  (x) :
  backward.to_fun f_deg hm
    (forward.to_fun 𝒜 f_deg x) = x :=
begin
  haveI : Π (i : ℕ) (x : (𝒜 i)), decidable (x ≠ 0) := λ _ _, classical.dec _,
  haveI : decidable_eq (away f) := classical.dec_eq _,
  ext z, split; intros hz,
  { change ∀ i, _ at hz,
    ...
    intros i hi,
    specialize hz i,
    erw forward.mem_carrier_iff at hz,
    dsimp only at hz,
    rw ←graded_algebra.proj_apply,
    erw [←ideal.submodule_span_eq, finsupp.span_eq_range_total, set.mem_range] at hz,
    obtain ⟨c, eq1⟩ := hz,
...
lemma forward.to_fun_inj {f : A} {m : ℕ}
  (hm : 0 < m) (f_deg : f ∈ 𝒜 m) : function.injective (forward.to_fun 𝒜 f_deg) := λ x1 x2 hx12,
begin
  convert congr_arg (backward.to_fun f_deg hm) hx12; symmetry;
  apply backward_forward,
end

lemma forward.to_fun_surj {f : A} {m : ℕ}
  (hm : 0 < m) (f_deg : f ∈ 𝒜 m) : function.surjective (forward.to_fun 𝒜 f_deg) :=
begin
  erw function.surjective_iff_has_right_inverse,
  refine ⟨backward.to_fun f_deg hm, λ x, _⟩,
  rw forward_backward,
end
end backward_forward
...
def backward {f : A} {m : ℕ} (hm : 0 < m) (f_deg : f ∈ 𝒜 m) :
  (Spec.T (A⁰_ f_deg)) ⟶ (Proj.T| (pbo f)) :=
{ to_fun := backward.to_fun f_deg hm,
  continuous_to_fun := begin
    ...
    set set1 := forward.to_fun 𝒜 f_deg '' s with set1_eq,
    have o1 : is_open set1,
    {
      suffices : is_open (forward.to_fun 𝒜 f_deg '' {x | x.1 ∈ (pbo f).1 ⊓ (pbo a).1}),
      erw [set1_eq, set_eq1], exact this,

      have set_eq2 := calc forward.to_fun 𝒜 f_deg ''
            {x | x.1 ∈ (pbo f) ⊓ (pbo a)}
          = forward.to_fun 𝒜 f_deg ''
            {x | x.1 ∈ (pbo f) ⊓ (⨆ (i : ℕ), (pbo (proj 𝒜 i a)))}
          : begin
            congr',
            ext x,
      ...
      ... = forward.to_fun 𝒜 f_deg '' 
            {x | x.1 ∈
              (⨆ (i : ℕ), (pbo f) ⊓ (pbo (proj 𝒜 i a)) : opens Proj.T)}
          : begin
            congr',
            ext x,
            split; intros hx,
            { rcases hx with ⟨hx1, hx2⟩,
              ...
              refine ⟨(pbo f) ⊓ (pbo (proj 𝒜 j a)), ⟨j, rfl⟩, ⟨hx1, hx2⟩⟩, },
            { erw opens.mem_Sup at hx,
              obtain ⟨_, ⟨j, rfl⟩, ⟨hx1, hx2⟩⟩ := hx,
              refine ⟨hx1, _⟩,
              erw opens.mem_Sup,
              refine ⟨pbo (proj 𝒜 j a), ⟨j, rfl⟩, hx2⟩, },
          end
      ... = forward.to_fun 𝒜 f_deg '' ⋃ (i : ℕ), {x | x.1 ∈ ((pbo f) ⊓ (pbo (proj 𝒜 i a)))}
          : begin
            congr',
            ext x,
      ...
      ... = ⋃ (i : ℕ), forward.to_fun 𝒜 f_deg ''
              {x | x.1 ∈ ((pbo f) ⊓ (pbo (proj 𝒜 i a)))}
          : begin
            erw set.image_Union,
          end,
    ...
    apply is_open_Union,
    intros i,
    suffices : forward.to_fun 𝒜 f_deg '' {x | x.1 ∈ ((pbo f) ⊓ (pbo (proj 𝒜 i a)))}
        = (sbo (⟨mk ((proj 𝒜 i a)^m) ⟨f^i, ⟨_, rfl⟩⟩,
            ⟨i, ⟨(proj 𝒜 i a)^m, set_like.graded_monoid.pow_mem _ (submodule.coe_mem _)⟩, rfl⟩⟩ : A⁰_ f_deg)).1,
    { erw this,
      exact (prime_spectrum.basic_open _).2 },

    suffices : forward.to_fun 𝒜 f_deg ⁻¹' (sbo (⟨mk ((proj 𝒜 i a)^m) ⟨f^i, ⟨_, rfl⟩⟩,
            ⟨i, ⟨(proj 𝒜 i a)^m, set_like.graded_monoid.pow_mem _ (submodule.coe_mem _)⟩, rfl⟩⟩ : A⁰_ f_deg)).1 =
      {x | x.1 ∈ (pbo f) ⊓ (pbo (graded_algebra.proj 𝒜 i a))},
    { erw ←this,
      apply function.surjective.image_preimage,
      exact forward.to_fun_surj 𝒜 hm f_deg, },

    { erw forward.preimage_eq f_deg ((graded_algebra.proj 𝒜 i a)^m) i,
      erw projective_spectrum.basic_open_pow,
      exact hm } },

    suffices : set1 = backward.to_fun f_deg hm ⁻¹' _,
    erw ←this,
    exact o1,
    ...
    { erw set1_eq,
      ...
      { erw set.mem_preimage,
        ...
        erw backward_forward,
        exact α_mem, },
      { erw set.mem_preimage at hz,
        erw set.mem_image,
        refine ⟨backward.to_fun f_deg hm z, hz, _⟩,
        erw forward_backward, }, },
  end }
...
variables {𝒜}
def Top_component {f : A} {m : ℕ} (hm : 0 < m) (f_deg : f ∈ 𝒜 m) :
  (Proj.T| (pbo f)) ≅ (Spec.T (A⁰_ f_deg)) :=
{ hom := Top_component.forward m f_deg,
  inv := Top_component.backward hm f_deg,
  hom_inv_id' := begin
    ext1 x,
    simp only [id_app, comp_app],
    apply Top_component.backward_forward,
  end,
  inv_hom_id' := begin
    ext1 x,
    simp only [id_app, comp_app],
    apply Top_component.forward_backward,
  end }

end

namespace Sheaf_component
...
local notation `pf_sheaf` x := (Top_component hm f_deg).hom _* x.presheaf -- pushforward a sheaf
...
lemma inv_mem (y : unop U) :
  ((Top_component hm f_deg).inv y.1).1 ∈
    ((@opens.open_embedding Proj.T (pbo f)).is_open_map.functor.op.obj 
      ((opens.map (Top_component hm f_deg).hom).op.obj U)).unop :=
begin
  refine ⟨⟨((Top_component hm f_deg).inv y.1).1, ((Top_component hm f_deg).inv y.1).2⟩, _, rfl⟩,
  change _ ∈ _ ⁻¹' _,
  erw set.mem_preimage,
  change (Top_component.forward.to_fun 𝒜 f_deg (Top_component.backward.to_fun f_deg hm y.1)) ∈ _,
  erw Top_component.forward_backward 𝒜 hm f_deg y.1,
  exact y.2,
end

variable (hm)
def hl (y : unop U) : homogeneous_localization 𝒜 _ :=
hh.1 ⟨((Top_component hm f_deg).inv y.1).1, inv_mem y⟩
...
begin
  ...
  change _ ∉ (Top_component.backward.to_fun f_deg hm y.1).1.as_homogeneous_ideal at prop1,
  contrapose! prop1,
  change ∀ _, _,
  ...
  { -- n ≠ i, contradiction,
    by_contra ineq,
    simp only [proj_apply, decompose_of_mem_ne 𝒜 ((hl hm hh y).denom_mem) ineq, zero_pow hm, mk_zero] at hn,
    apply hn,
    exact submodule.zero_mem _, },
  apply hn,
  convert r,

  rw [proj_apply, ←eq1, decompose_of_mem_same],
...
begin
  ...
  erw [show (1 : localization.at_prime ((Top_component hm f_deg).inv y.1).1.as_homogeneous_ideal.to_ideal) =
    mk 1 1,
      begin
        symmetry,
        convert localization.mk_self _,
  ...
  rw [← hl.one] at eq1,
  have eq2 : proj 𝒜 ((hl hm 1 y).deg + j) ((hl hm 1 y).denom * c) 
    = proj 𝒜 ((hl hm 1 y).deg + j) ((hl hm 1 y).num * c),
  { exact congr_arg _ eq1, },
  
  have eq3 : proj 𝒜 ((hl hm 1 y).deg + j) ((hl hm 1 y).denom * c)
    = (hl hm 1 y).denom * (proj 𝒜 j c),
  ...
  have eq4 : proj 𝒜 ((hl hm 1 y).deg + j)
    ((hl hm 1 y).num * c)
    = (hl hm 1 y).num * (proj 𝒜 j c),
  { apply graded_algebra.proj_hom_mul,
  ...
  congr' 1,
  exact calc (hl hm 1 y).num * (hl hm 1 y).denom ^ m.pred * (proj 𝒜 j) c ^ m
          = (hl hm 1 y).num * (hl hm 1 y).denom ^ m.pred * (proj 𝒜 j) c ^ (m.pred + 1)
          : begin
            congr',
            symmetry,
      ...
      ... = (hl hm 1 y).num * (hl hm 1 y).denom ^ m.pred * ((proj 𝒜 j) c ^ m.pred * proj 𝒜 j c)
          : by ring_exp
      ... = ((hl hm 1 y).num * proj 𝒜 j c) * ((hl hm 1 y).denom ^ m.pred * (proj 𝒜 j) c ^ m.pred)
          : by ring
      ... = ((hl hm 1 y).denom * proj 𝒜 j c) * ((hl hm 1 y).denom ^ m.pred * (proj 𝒜 j) c ^ m.pred)
          : by rw eq2
      ... = ((hl hm 1 y).denom * proj 𝒜 j c) ^ (1 + m.pred)
          : by ring_exp
      ... = ((hl hm 1 y).denom * proj 𝒜 j c) ^ m
          : begin
            congr' 1,
            rw [add_comm],
...
begin
  ...
  erw [show (0 : localization.at_prime ((Top_component hm f_deg).inv y.1).1.as_homogeneous_ideal.to_ideal) =
    localization.mk 0 1,
      begin
        rw localization.mk_zero,
      end, localization.mk_eq_mk', is_localization.eq] at eq1,
  ...
  change c ∉ Top_component.backward.carrier _ at hc1,
  change ¬(∀ i : ℕ, _ ∈ _) at hc1,
  rw not_forall at hc1,
  obtain ⟨j, hc1⟩ := hc1,
  replace eq1 := eq1.symm,
  have eq2 : proj 𝒜 ((hl hm 0 y).deg + j) ((hl hm 0 y).num * c) = 0,
  { erw [eq1, linear_map.map_zero], },
  have eq3 : proj 𝒜 ((hl hm 0 y).deg + j) ((hl hm 0 y).num * c)
  ...
  dsimp only,
  convert mk_zero _,
  exact calc (hl hm 0 y).num * (hl hm 0 y).denom ^ m.pred * (proj 𝒜 j) c ^ m
          = (hl hm 0 y).num * (hl hm 0 y).denom ^ m.pred * (proj 𝒜 j) c ^ (m.pred + 1)
          : begin
            congr',
            symmetry,
            apply nat.succ_pred_eq_of_pos,
      ...
      ... = (hl hm 0 y).num * (hl hm 0 y).denom ^ m.pred * ((proj 𝒜 j) c ^ m.pred * proj 𝒜 j c)
          : by rw [pow_add, pow_one]
      ... = ((hl hm 0 y).num * proj 𝒜 j c)
            * ((hl hm 0 y).denom ^ m.pred * (proj 𝒜 j) c ^ m.pred) : by ring
      ... = 0 * ((hl hm 0 y).denom ^ m.pred * (proj 𝒜 j) c ^ m.pred) : by rw eq2
      ... = 0 : by rw zero_mul,
end
...
begin
  ...
  use localization.mk ((proj 𝒜 j c)^m) ⟨f^j, ⟨_, rfl⟩⟩,
...
⟨homeo_of_iso (Top_component hm f_deg) '' {z | z.1 ∈ V.1}, begin
...
lemma mem_open_subset_of_inv_mem (V : opens Proj.T) (y : unop U)
  (mem1 : (((Top_component hm f_deg).inv) y.val).val ∈ V) :
  y.1 ∈ open_set 𝒜 hm f_deg V  :=
begin
  refine ⟨(Top_component hm f_deg).inv y.1, mem1, _⟩,
  rw [homeo_of_iso_apply],
  convert Top_component.forward_backward _ _ _ _,
end
...
lemma not_mem
  ...
  ∉ ((homeo_of_iso (Top_component hm f_deg)) z).as_ideal := λ rid,
begin
  haveI : decidable_eq (away f) := classical.dec_eq _,

  rw homeo_of_iso_apply at rid,
  erw Top_component.forward.mem_carrier_iff at rid,
  dsimp only at rid,
...
lemma mk_proj_pow_not_mem
  ...
  ideal.span
    {y : localization (submonoid.powers f) | ∃ (g : A),
      g ∈ (projective_spectrum.as_homogeneous_ideal z.val).to_ideal ∧ y = localization.mk g 1} :=
begin
  haveI : decidable_eq (away f) := classical.dec_eq _,
...
lemma inv_hom_mem_bo (V : opens Proj.T) (z : Proj.T| (pbo f))
  ...
  (((Top_component hm f_deg).inv)
    (subset2 ⟨(homeo_of_iso (Top_component hm f_deg)) z, begin
    erw [set.mem_preimage],
    refine ⟨z, z_mem, rfl⟩,
...
begin
  ...
  change _ ∈ ((homeo_of_iso (Top_component hm f_deg)) z).1 at rid,
  have rid2 : (1 : A⁰_ f_deg) ∈ ((homeo_of_iso (Top_component hm f_deg)) z).1,
  { convert rid,
    rw subtype.ext_iff_val,
    dsimp only,
  ...
  apply (((Top_component hm f_deg).hom) z).is_prime.1,
  rw ideal.eq_top_iff_one,
  exact rid2,
end
...
begin
  ...
  set z' := (((Top_component hm f_deg).inv)
    (subset2 ⟨(homeo_of_iso (Top_component hm f_deg)) z, begin
    erw [set.mem_preimage],
    refine ⟨z, z_mem, rfl⟩,
  ...
  suffices : ((Top_component hm f_deg).hom) ⟨z', _⟩ ∈ open_set 𝒜 hm f_deg V,
  apply subset3,
  exact this,
  ...
  dsimp only,
  rw z'_eq,
  change z.1 = (Top_component.backward hm f_deg (Top_component.forward _ _ _)).1,
  congr', 
  symmetry,
  apply Top_component.backward_forward 𝒜 hm f_deg z,
end
...
begin
  ...
  obtain ⟨V, mem1, subset1, degree, ⟨a, a_mem⟩, ⟨b, b_mem⟩, eq1⟩ := hh.2 ⟨((Top_component hm f_deg).inv y.1).1, inv_mem y⟩,
  ...
  { by_contra rid,
    ...
    apply rid, },
  obtain ⟨j, hj⟩ := mem_C,
  refine ⟨⟨⟨localization.mk ((proj 𝒜 j C)^m) ⟨f^j, ⟨_, rfl⟩⟩,
    ⟨j, ⟨(proj 𝒜 j C)^m, set_like.graded_monoid.pow_mem _ (submodule.coe_mem _)⟩, rfl⟩⟩, _⟩, _⟩,
  { change _ ∉ _,
    simp only [← subtype.val_eq_coe],
    erw Top_component.forward.mem_carrier_iff,
  ...
  { rw subtype.ext_iff_val,
    change z.1 = (Top_component.backward hm f_deg (Top_component.forward m f_deg _)).1,
    congr', 
    symmetry,
    apply Top_component.backward_forward 𝒜 hm f_deg z, },
  erw [eq_pt] at eq1,
...
def forward {f : A} {m : ℕ} (hm : 0 < m) (f_deg : f ∈ 𝒜 m):
  ((Top_component hm f_deg).hom _* (Proj| (pbo f)).presheaf) ⟶ (Spec (A⁰_ f_deg)).presheaf :=
{ app := λ U, forward.to_fun hm f_deg U,
...
variables (y : ((@opens.open_embedding Proj.T (pbo f)).is_open_map.functor.op.obj 
  ((opens.map (Top_component hm f_deg).hom).op.obj V)).unop)
...
lemma data_prop2 :
  (Top_component hm f_deg).hom ⟨y.1, data_prop1 hm f_deg V y⟩ ∈ unop V :=
begin
...
def data : structure_sheaf.localizations (A⁰_ f_deg) 
  ((Top_component hm f_deg).hom ⟨y.1, data_prop1 _ _ _ _⟩) :=
...
private lemma data.exist_rep 
  (data : structure_sheaf.localizations (A⁰_ f_deg) ((Top_component hm f_deg).hom ⟨y.1, data_prop1 _ _ _ _⟩)) :
  ∃ (a : A⁰_ f_deg) (b : ((Top_component hm f_deg).hom ⟨y.1, data_prop1 _ _ _ _⟩).as_ideal.prime_compl),
  data = mk a b :=
begin
...
lemma data.denom_not_mem : 
  (data.denom hm f_deg hh y) ∉ ((Top_component hm f_deg).hom ⟨y.1, data_prop1 _ _ _ _⟩).as_ideal :=
...
def num : A :=
degree_zero_part.num f_deg (data.num hm f_deg hh y) * f^(degree_zero_part.deg f_deg (data.denom hm f_deg hh y))

lemma num.mem :
  (num hm f_deg hh y) ∈ 
    𝒜 (m * (degree_zero_part.deg f_deg (data.num hm f_deg hh y)) 
      + m * (degree_zero_part.deg f_deg (data.denom hm f_deg hh y))) :=
mul_mem (degree_zero_part.num_mem f_deg _) $ begin
  convert (set_like.graded_monoid.pow_mem (degree_zero_part.deg f_deg (data.denom hm f_deg hh y)) f_deg) using 1,
  rw mul_comm,
  refl,
end

def denom : A :=
degree_zero_part.num f_deg (data.denom hm f_deg hh y) * f^(degree_zero_part.deg f_deg (data.num hm f_deg hh y))
...
begin
  ...
  apply set_like.graded_monoid.mul_mem,
  { rw mul_comm,
    exact set_like.graded_monoid.pow_mem (degree_zero_part.deg f_deg (data.num hm f_deg hh y)) f_deg, },
...
begin
  ...
  { have mem1 := data.denom_not_mem hm f_deg hh y,
    have eq1 := degree_zero_part.eq f_deg (data.denom hm f_deg hh y),
    dsimp only at mem1,
    change _ ∉ _ at mem1,
    apply mem1,
    change
      (data.denom hm f_deg hh y) ∈ ((Top_component.forward.carrier f_deg) ⟨y.1, _⟩),
    rw Top_component.forward.mem_carrier_iff,
    rw eq1,
    convert ideal.mul_mem_left _ _ _,
    work_on_goal 2
    { exact mk 1 ⟨f^degree_zero_part.deg f_deg (data.denom hm f_deg hh y), ⟨_, rfl⟩⟩ },
    work_on_goal 2
    { exact mk (degree_zero_part.num f_deg (data.denom hm f_deg hh y)) 1 },
    { rw [mk_mul, one_mul, mul_one], },
    { apply ideal.subset_span,
      exact ⟨_, H1, rfl⟩ }, },
...
{ deg := m * (degree_zero_part.deg f_deg (data.num hm f_deg hh y)) 
      + m * (degree_zero_part.deg f_deg (data.denom hm f_deg hh y)),
...
begin
  ...
  rw [show (1 : structure_sheaf.localizations (A⁰_ f_deg)
    (((Top_component hm f_deg).hom) ⟨y.val, y_mem⟩)) = localization.mk 1 1,
  ...
  simp only [localization.mk_eq_mk', is_localization.eq],
  change _ ∉ _ at hC,
  erw Top_component.forward.mem_carrier_iff at hC,
  dsimp only at C_degree_zero hC,

  have eq_num := degree_zero_part.eq f_deg (data.num hm f_deg 1 y),
  have eq_denom := degree_zero_part.eq f_deg (data.denom hm f_deg 1 y),
  ...
  rw calc degree_zero_part.num f_deg (data.num hm f_deg 1 y)
        * f ^ degree_zero_part.deg f_deg (data.denom hm f_deg 1 y)
        * (C * (f ^ l * f ^ n1))
      = degree_zero_part.num f_deg (data.num hm f_deg 1 y) * C
        * f ^ (degree_zero_part.deg f_deg (data.denom hm f_deg 1 y) + l)
        * f^n1 : by ring_exp,
  rw [eq1, pow_add],
  ring,
end
...
begin
  ...
  erw [show (0 : structure_sheaf.localizations (A⁰_ f_deg)
    (((Top_component hm f_deg).hom) ⟨y.val, y_mem⟩)) = localization.mk 0 1,
  ...
  have eq_num := degree_zero_part.eq _ (data.num hm f_deg 0 y),
  have eq_denom := degree_zero_part.eq _ (data.denom hm f_deg 0 y),
  ...
  rw calc degree_zero_part.num f_deg (data.num hm f_deg 0 y)
        * f ^ degree_zero_part.deg f_deg (data.denom hm f_deg 0 y)
        * (C * f ^ n1)
      = degree_zero_part.num f_deg (data.num hm f_deg 0 y)
        * C * f ^ n1
        * f ^ degree_zero_part.deg f_deg (data.denom hm f_deg 0 y)
      : by ring,
  rw [eq1, zero_mul],
...
begin
  ...
  set a_xy : A := degree_zero_part.num f_deg (data.num hm f_deg (x + y) z) with a_xy_eq,
  set i_xy : ℕ := degree_zero_part.deg f_deg (data.num hm f_deg (x + y) z) with i_xy_eq,
  set b_xy : A := degree_zero_part.num f_deg (data.denom hm f_deg (x + y) z) with b_xy_eq,
  set j_xy : ℕ := degree_zero_part.deg f_deg (data.denom hm f_deg (x + y) z) with j_xy_eq,

  set a_x : A := degree_zero_part.num f_deg (data.num hm f_deg x z) with a_x_eq,
  set i_x : ℕ := degree_zero_part.deg f_deg (data.num hm f_deg x z) with i_x_eq,
  set b_x : A := degree_zero_part.num f_deg (data.denom hm f_deg x z) with b_x_eq,
  set j_x : ℕ := degree_zero_part.deg f_deg (data.denom hm f_deg x z) with j_x_eq,

  set a_y : A := degree_zero_part.num f_deg (data.num hm f_deg y z) with a_y_eq,
  set i_y : ℕ := degree_zero_part.deg f_deg (data.num hm f_deg y z) with i_y_eq,
  set b_y : A := degree_zero_part.num f_deg (data.denom hm f_deg y z) with b_y_eq,
  set j_y : ℕ := degree_zero_part.deg f_deg (data.denom hm f_deg y z) with j_y_eq,
...
begin
  ...
  simp only [degree_zero_part.mul_val, degree_zero_part.add_val,
    show ∀ (α β : (prime_spectrum.as_ideal (((Top_component hm f_deg).hom)
...
lemma hom_apply_mem :
  (Top_component hm f_deg).hom ⟨y.1, mem_pbo hm f_deg y⟩ ∈ unop V := 
begin
  obtain ⟨a, ha1, ha2⟩ := y.2,
  erw set.mem_preimage at ha1,
  change ((Top_component hm f_deg).hom ⟨y.1, _⟩) ∈ (unop V).1,
  convert ha1,
  rw subtype.ext_iff_val,
  exact ha2.symm,
end
...
⟨{x | ∃ x' : homeo_of_iso (Top_component hm f_deg) ⁻¹' VV.1, x = x'.1.1}, begin
  have O1 := (homeomorph.is_open_preimage (homeo_of_iso (Top_component hm f_deg))).2 VV.2,
  ...
  { obtain ⟨x', rfl⟩ := hγ,
    ...
    intro rid,
    have mem3 : (⟨localization.mk f ⟨f^1, ⟨_, rfl⟩⟩, ⟨1, ⟨_, by simpa [mul_one] using f_deg⟩, rfl⟩⟩ : A⁰_ f_deg) ∈ ((Top_component hm f_deg).hom x'.1).as_ideal,
    { erw Top_component.forward.mem_carrier_iff,
      ...
      { apply ideal.subset_span,
        refine ⟨f, rid, rfl⟩, } },
    have mem4 : (1 : A⁰_ f_deg) ∈ ((Top_component hm f_deg).hom x'.1).as_ideal,
    { convert mem3,
      rw [subtype.ext_iff_val, degree_zero_part.one_val],
      dsimp only,
      symmetry,
      ...
      dsimp only,
      rw pow_one, },
    apply ((Top_component hm f_deg).hom x'.1).is_prime.1,
    rw ideal.eq_top_iff_one,
    exact mem4, },
...
begin
  ...
  have hom_y_mem : (Top_component hm f_deg).hom ⟨y.1, y_mem⟩ ∈ unop V,
  { convert is_locally_quotient.hom_apply_mem hm f_deg y, },
  have is_local := hh.2,
  rw structure_sheaf.is_locally_fraction_pred' at is_local,
  specialize is_local ⟨(Top_component hm f_deg).hom ⟨y.1, y_mem⟩, hom_y_mem⟩,
  ...
  have hom_z_mem_VV : ((Top_component hm f_deg).hom) ⟨z, z_mem_bo⟩ ∈ VV,
  { obtain ⟨γ, h1, h2⟩ := z_mem_U,
    have mem1 := γ.2,
    erw set.mem_preimage at mem1,
    exact mem1, },

  specialize is_local ⟨((Top_component hm f_deg).hom ⟨z, z_mem_bo⟩), hom_z_mem_VV⟩,
  ...
  have data_eq : data hm f_deg hh (subset2 ⟨z, z_mem_U⟩) =
    hh.val (subset1 ⟨((Top_component hm f_deg).hom) ⟨z, z_mem_bo⟩, hom_z_mem_VV⟩),
  { congr', },
  rw ←data_eq at eq1,
  ...
  set p := degree_zero_part.num f_deg (data.num hm f_deg hh ⟨z, z_mem2⟩) with p_eq,
  set q := degree_zero_part.num f_deg (data.denom hm f_deg hh ⟨z, z_mem2⟩) with q_eq,
  set ii := degree_zero_part.deg f_deg (data.num hm f_deg hh ⟨z, z_mem2⟩) with ii_eq,
  set jj := degree_zero_part.deg f_deg (data.denom hm f_deg hh ⟨z, z_mem2⟩) with jj_eq,
...
def to_fun.aux (hh : (Spec (A⁰_ f_deg)).presheaf.obj V) : ((Top_component hm f_deg).hom _* (Proj| (pbo f)).presheaf).obj V :=
...
def to_fun : (Spec (A⁰_ f_deg)).presheaf.obj V ⟶ ((Top_component hm f_deg).hom _* (Proj| (pbo f)).presheaf).obj V :=
...
def backward {f : A} {m : ℕ} (hm : 0 < m) (f_deg : f ∈ 𝒜 m) : 
  (Spec (A⁰_ f_deg)).presheaf ⟶ (Top_component hm f_deg).hom _* (Proj| (pbo f)).presheaf :=
{ app := λ V, backward.to_fun hm f_deg V,
  naturality' := λ U V subset1, begin
    ext1 z,
    simp only [comp_apply, ring_hom.coe_mk, functor.op_map, presheaf.pushforward_obj_map],
    refl,
...
namespace backward_forward

variables {𝒜} {m : ℕ} {f : A} (hm : 0 < m) (f_deg : f ∈ 𝒜 m) (V : (opens (Spec.T (A⁰_ f_deg)))ᵒᵖ)
variables (hh : ((Top_component hm f_deg).hom _* (Proj| (pbo f)).presheaf).obj V)
variables (z : (((@opens.open_embedding Proj.T (pbo f)).is_open_map.functor.op.obj ((opens.map (Top_component hm f_deg).hom).op.obj V)).unop))
...
lemma inv_hom_apply_eq :
  ((Top_component hm f_deg).inv ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg _ _⟩)).1 = z.1 :=
begin
  change (Top_component.backward.to_fun f_deg hm (Top_component.forward.to_fun 𝒜 f_deg _)).1 = z.1,
  rw Top_component.backward_forward,
end

lemma pt_eq :
  z = ⟨((Top_component hm f_deg).inv ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg _ _⟩)).1, begin
...
lemma C_not_mem (C : A) (L1 : ℕ) (C_mem : C ∈ 𝒜 (m * L1))
  (hC : (⟨localization.mk C ⟨f ^ L1, ⟨_, rfl⟩⟩, ⟨L1, ⟨_, C_mem⟩, rfl⟩⟩ : A⁰_ f_deg) ∉ 
    ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg V _⟩).as_ideal) :
...
lemma C_not_mem2
  ...
  (hC : (⟨localization.mk C ⟨f ^ L1, ⟨_, rfl⟩⟩, ⟨L1, ⟨_, C_mem⟩, rfl⟩⟩ : A⁰_ f_deg) ∉ 
    ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg V _⟩).as_ideal)
  (β : A) 
  (β_not_in : β ∉ (((Top_component hm f_deg).inv)
      ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg V _⟩)).1.as_homogeneous_ideal) :
...
begin
  ...
  replace H2 := z.1.is_prime.mem_of_pow_mem _ H2,
  apply β_not_in,
  have eq1 : (((Top_component hm f_deg).inv) ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg V _⟩)).1 = z.1,
  { change (Top_component.backward.to_fun f_deg hm (Top_component.forward.to_fun 𝒜 _ _)).1 = z.1,
    rw Top_component.backward_forward, },
  erw eq1,
  exact H2,
...
omit hm
lemma _root_.algebraic_geometry.Sheaf_component.backward_forward :
  backward.bmk hm f_deg V
    (((forward 𝒜 hm f_deg).app V) hh) z = hh.1 z :=
begin
  unfold backward.bmk,
  rw [homogeneous_localization.ext_iff_val, homogeneous_localization.val_mk'],
  simp only [← subtype.val_eq_coe],

  set hom_z := (Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg V _⟩ with hom_z_eq,
  have hom_z_mem_V : hom_z ∈ unop V,
  { apply backward.data_prop2 hm f_deg V _, },

  set data := backward.data hm f_deg (((forward 𝒜 hm f_deg).app V) hh) z with data_eq,
  have data_eq1 := data_eq,
  replace data_eq1 : data = forward.fmk hm hh ⟨hom_z, hom_z_mem_V⟩,
  { convert data_eq1, },
  unfold forward.fmk forward.num forward.denom at data_eq1,

  have data_eq2 := backward.data.eq_num_div_denom hm f_deg (((forward 𝒜 hm f_deg).app V) hh) z,
  rw [←data_eq, data_eq1] at data_eq2,
  set α := (hh.1 ⟨((Top_component hm f_deg).inv hom_z).1, forward.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).num with α_eq,
  set β := (hh.1 ⟨((Top_component hm f_deg).inv hom_z).1, forward.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).denom with β_eq,
  set ι := (hh.1 ⟨((Top_component hm f_deg).inv hom_z).1, forward.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).deg with ι_eq,
  have β_not_in : β ∉ (((Top_component hm f_deg).inv)
      ((Top_component hm f_deg).hom ⟨z.1, backward.data_prop1 hm f_deg V _⟩)).1.as_homogeneous_ideal,
  { exact (hh.1 ⟨((Top_component hm f_deg).inv hom_z).1, forward.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).denom_not_mem, },
  have hartshorne_eq : (hh.1 ⟨((Top_component hm f_deg).inv hom_z).1, forward.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).val
    = localization.mk α ⟨β, β_not_in⟩,
  { exact (hh.1 ⟨((Top_component hm f_deg).inv hom_z).1, forward.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).eq_num_div_denom, },
  ...
  set a := degree_zero_part.num f_deg (backward.data.num hm f_deg (((forward 𝒜 hm f_deg).app V) hh) z) with a_eq,
  set b := degree_zero_part.num f_deg (backward.data.denom hm f_deg (((forward 𝒜 hm f_deg).app V) hh) z) with b_eq,
  set ii := degree_zero_part.deg f_deg (backward.data.num hm f_deg (((forward 𝒜 hm f_deg).app V) hh) z) with ii_eq,
  set jj := degree_zero_part.deg f_deg (backward.data.denom hm f_deg (((forward 𝒜 hm f_deg).app V) hh) z) with jj_eq,
  ...
  simp only [←subtype.val_eq_coe, show ∀ (p q : submonoid.powers f), (p * q).1 = p.1 * q.1, from λ _ _, rfl,
    ←pow_add] at data_eq2,
  unfold backward.num backward.denom,
  dsimp only,
  rw [localization.mk_eq_mk', is_localization.eq],
...
namespace forward_backward

variables {𝒜} {m : ℕ} {f : A} (hm : 0 < m) (f_deg : f ∈ 𝒜 m) (V : (opens ((Spec.T (A⁰_ f_deg))))ᵒᵖ)
variables  (hh : ((Spec (A⁰_ f_deg)).presheaf.obj V)) (z : V.unop)

lemma inv_mem :
((Top_component hm f_deg).inv z).1 ∈
  ((@opens.open_embedding Proj.T (pbo f)).is_open_map.functor.op.obj
    ((opens.map (Top_component hm f_deg).hom).op.obj V)).unop :=
begin
  have mem1 := ((Top_component hm f_deg).inv z).2,
  refine ⟨((Top_component hm f_deg).inv z), _, rfl⟩,
  erw set.mem_preimage,
  convert z.2,
  convert Top_component.forward_backward _ _ _ _,
end
...
lemma inv_mem_pbo :
    ((Top_component hm f_deg).inv z).1 ∈ pbo f :=
begin
  intro rid,
  obtain ⟨⟨a, ha1⟩, ha2, ha3⟩ := inv_mem hm f_deg V z,
  change a = ((Top_component hm f_deg).inv z).1 at ha3,
  erw ←ha3 at rid,
  apply ha1,
  exact rid,
end
...
lemma dd_not_mem_z
  (dd : (prime_spectrum.as_ideal
    (((Top_component hm f_deg).hom) ⟨((Top_component hm f_deg).inv z).1, inv_mem_pbo hm f_deg V z⟩)).prime_compl) :
  dd.1 ∉ z.1.as_ideal :=
begin
  have mem1 := dd.2,
  change dd.1 ∉ (((Top_component hm f_deg).hom) ⟨((Top_component hm f_deg).inv z).val, _⟩).as_ideal at mem1,
  convert mem1,
  change z.1 = Top_component.forward.to_fun 𝒜 f_deg (Top_component.backward.to_fun f_deg hm _),
  rw Top_component.forward_backward,
  refl,
end
...
lemma eq0
  ...
  (data_eq1 : localization.mk nn dd =
    hh.val ⟨((Top_component hm f_deg).hom)
    ⟨((Top_component hm f_deg).inv z).val, _⟩, begin
      convert z.2,
      change (Top_component.forward.to_fun 𝒜 f_deg (Top_component.backward.to_fun f_deg hm _)) = z.1,
      rw Top_component.forward_backward,
      refl,
    end⟩) :
  (hh.1 z) = localization.mk nn ⟨dd.1, dd_not_mem_z hm f_deg V z dd⟩ :=
begin
  convert backward_forward.section_congr f_deg V hh _ _ _ nn ⟨dd.1, _⟩ _,
  refine ⟨((Top_component hm f_deg).hom) ⟨(((Top_component hm f_deg).inv) ↑z).val, _⟩, _⟩,
  apply inv_mem_pbo,
  convert z.2,
  convert Top_component.forward_backward _ _ _ _,
  rw subtype.ext_iff_val,
...
lemma not_mem1
  (C : A) (j : ℕ) (hj : (proj 𝒜 j) C ∉ (((Top_component hm f_deg).inv z)).1.as_homogeneous_ideal) :
  (⟨localization.mk ((proj 𝒜 j) C ^ m) ⟨f ^ j, ⟨j, rfl⟩⟩,
    ⟨j, ⟨(proj 𝒜 j C)^m, set_like.graded_monoid.pow_mem m (submodule.coe_mem _)⟩, rfl⟩⟩ : A⁰_ f_deg) ∈
  (prime_spectrum.as_ideal z.val).prime_compl :=
begin
  intro rid,
  change proj 𝒜 j C ∉ Top_component.backward.carrier _ at hj,
  apply hj,
  intro k,
  by_cases ineq : j = k,
  { rw ←ineq,
...
lemma eq1
  (hart : homogeneous_localization 𝒜 ((Top_component hm f_deg).inv z).1.as_homogeneous_ideal.to_ideal)
  (C : A) (j : ℕ) (hj : (graded_algebra.proj 𝒜 j) C ∉
    projective_spectrum.as_homogeneous_ideal (((Top_component hm f_deg).inv z)).val)
  (dd : (prime_spectrum.as_ideal
   (((Top_component hm f_deg).hom) ⟨((Top_component hm f_deg).inv z).1, inv_mem_pbo hm f_deg V z⟩)).prime_compl)
  (nn : A⁰_ f_deg)
  (EQ : hart.num * (degree_zero_part.num f_deg dd.val * f ^ degree_zero_part.deg f_deg nn) * graded_algebra.proj 𝒜 j C =
        degree_zero_part.num f_deg nn * f ^ degree_zero_part.deg f_deg dd.val * hart.denom * graded_algebra.proj 𝒜 j C) :
  hart.num * hart.denom ^ m.pred * degree_zero_part.num f_deg dd.val * (graded_algebra.proj 𝒜 j) C ^ m *
    f ^ (degree_zero_part.deg f_deg nn + hart.deg + j) =
  degree_zero_part.num f_deg nn * hart.denom ^ m * (graded_algebra.proj 𝒜 j) C ^ m *
    f ^ (hart.deg + degree_zero_part.deg f_deg dd.val + j) :=
begin
  rw calc hart.num * hart.denom ^ m.pred * degree_zero_part.num f_deg dd.val
            * (graded_algebra.proj 𝒜 j) C ^ m * f ^ (degree_zero_part.deg f_deg nn + hart.deg + j)
          = hart.num * hart.denom ^ m.pred * degree_zero_part.num f_deg dd.val
            * (graded_algebra.proj 𝒜 j) C ^ (m.pred + 1) * f ^ (degree_zero_part.deg f_deg nn + hart.deg + j)
          : begin
            congr',
            symmetry,
            apply nat.succ_pred_eq_of_pos hm,
          end
      ... = hart.num * hart.denom ^ m.pred * degree_zero_part.num _ dd.val
            * ((graded_algebra.proj 𝒜 j) C ^ m.pred * graded_algebra.proj 𝒜 j C)
            * f ^ (degree_zero_part.deg f_deg nn + hart.deg + j) : by simp only [pow_add, pow_one]
      ... = hart.num * hart.denom ^ m.pred * degree_zero_part.num _ dd.val
            * ((graded_algebra.proj 𝒜 j) C ^ m.pred * graded_algebra.proj 𝒜 j C)
            * (f ^ degree_zero_part.deg f_deg nn * f ^ hart.deg * f^j) : by simp only [pow_add]
      ... = (hart.num * (degree_zero_part.num _ dd.val * f ^ degree_zero_part.deg f_deg nn) * graded_algebra.proj 𝒜 j C)
            * (hart.denom ^ m.pred * graded_algebra.proj 𝒜 j C ^ m.pred * f ^ hart.deg * f ^ j) : by ring
      ... = (degree_zero_part.num _ nn * f ^ degree_zero_part.deg f_deg dd.val * hart.denom * graded_algebra.proj 𝒜 j C)
            * (hart.denom ^ m.pred * graded_algebra.proj 𝒜 j C ^ m.pred * f ^ hart.deg * f ^ j) : by rw EQ
      ... = (degree_zero_part.num _ nn * f ^ degree_zero_part.deg f_deg dd.val)
            * (graded_algebra.proj 𝒜 j C ^ m.pred * graded_algebra.proj 𝒜 j C)
...
lemma eq2
  ...
  (dd : (prime_spectrum.as_ideal
   (((Top_component hm f_deg).hom) ⟨((Top_component hm f_deg).inv z).1, inv_mem_pbo hm f_deg V z⟩)).prime_compl)
  (nn : A⁰_ f_deg)
  (eq1 : hart.num * (degree_zero_part.num f_deg dd.val * f ^ degree_zero_part.deg f_deg nn) * C =
    degree_zero_part.num f_deg nn * f ^ degree_zero_part.deg f_deg dd.val * hart.denom * C) :
  hart.num * (degree_zero_part.num f_deg dd.val * f ^ degree_zero_part.deg f_deg nn) * graded_algebra.proj 𝒜 j C =
  degree_zero_part.num f_deg nn * f ^ degree_zero_part.deg f_deg dd.val * hart.denom * graded_algebra.proj 𝒜 j C :=
begin
  have mem1 := degree_zero_part.num_mem _ dd.1,
  have mem2 := degree_zero_part.num_mem _ nn,
  have eq2 := congr_arg
    (graded_algebra.proj 𝒜 (hart.deg + m * degree_zero_part.deg _ dd.1 + m * degree_zero_part.deg _ nn + j)) eq1,
  erw graded_algebra.proj_hom_mul at eq2,
  erw graded_algebra.proj_hom_mul at eq2,
  exact eq2,

  rw show degree_zero_part.num _ nn * f ^ degree_zero_part.deg _ dd.val * hart.denom =
    hart.denom * f ^ degree_zero_part.deg _ dd.1 * degree_zero_part.num _ nn, by ring,
  apply set_like.graded_monoid.mul_mem,
  apply set_like.graded_monoid.mul_mem,
...
lemma forward_backward {m : ℕ} {f : A} (f_deg : f ∈ 𝒜 m) (hm : 0 < m) (V hh z) :
  forward.fmk hm (((backward 𝒜 hm f_deg).app V) hh) z =
  hh.val z :=
begin
  haveI : Π (i : ℕ) (x : ↥(𝒜 i)), decidable (x ≠ 0) := λ _ _, classical.dec _,

  set b_hh := ((backward 𝒜 hm f_deg).app V hh) with b_hh_eq,
  unfold forward.fmk forward.num forward.denom,
  set inv_z := ((Top_component hm f_deg).inv z) with inv_z_eq,
  have inv_z_mem : inv_z.1 ∈
    ((@opens.open_embedding Proj.T (pbo f)).is_open_map.functor.op.obj
    ((opens.map (Top_component hm f_deg).hom).op.obj V)).unop,
  { apply forward_backward.inv_mem, },

  have inv_z_mem_bo : inv_z.1 ∈ projective_spectrum.basic_open 𝒜 f,
  { apply forward_backward.inv_mem_pbo, },
  ...
  replace hart_eq : hart.val = (backward.bmk hm f_deg V hh ⟨inv_z.val, inv_z_mem⟩).val,
  { convert hart_eq },
  unfold backward.bmk at hart_eq,
  rw [homogeneous_localization.val_mk'] at hart_eq,
  simp only [← subtype.val_eq_coe] at hart_eq,
  unfold backward.num backward.denom at hart_eq,

  set data := backward.data hm f_deg hh ⟨inv_z.val, inv_z_mem⟩ with data_eq,
  have data_eq1 := data_eq,
  unfold backward.data at data_eq1,
  erw backward.data.eq_num_div_denom at data_eq,
  erw data_eq at data_eq1,
  set nn := backward.data.num hm f_deg hh ⟨inv_z.val, inv_z_mem⟩ with nn_eq,
  set dd := backward.data.denom hm f_deg hh ⟨inv_z.val, inv_z_mem⟩ with dd_eq,
  ...
  { apply forward_backward.dd_not_mem_z, },

  have eq0 : (hh.1 z) = localization.mk nn ⟨dd, dd_not_mem_z⟩,
  { convert forward_backward.eq0 hm f_deg _ hh z ⟨dd, _⟩ nn data_eq1, },
  ...
  refine ⟨⟨⟨localization.mk ((graded_algebra.proj 𝒜 j C)^m) ⟨f^j, ⟨j, rfl⟩⟩,
    ⟨j, ⟨(proj 𝒜 j C)^m, set_like.graded_monoid.pow_mem _ (submodule.coe_mem _)⟩, rfl⟩⟩,
    forward_backward.not_mem1 hm f_deg V z C j hj⟩, _⟩,
  { rw [localization.mk_mul, localization.mk_mul, localization.mk_eq_mk', is_localization.eq],
    ...
    apply forward_backward.eq1,
    exact hj,
    apply forward_backward.eq2;
    assumption, }
end
...
def Sheaf_component {m : ℕ} {f : A} (f_deg : f ∈ 𝒜 m) (hm : 0 < m) :
  (Top_component hm f_deg).hom _* (Proj| (pbo f)).presheaf ≅ (Spec (A⁰_ f_deg)).presheaf :=
{ hom := Sheaf_component.forward 𝒜 hm f_deg,
  inv := Sheaf_component.backward 𝒜 hm f_deg,
  hom_inv_id' := begin
    ext1,
    ext1 V,
    ext1 hh,
    ...
    ext1 z,
    apply Sheaf_component.backward_forward,
  end,
  inv_hom_id' := begin
    ext1, ext1 V, ext1 hh,
    ...
    rw subtype.ext_iff_val,
    ext1 z,
    apply Sheaf_component.forward_backward,
  end }

def SheafedSpace.iso_of_PresheafedSpace_iso {C : Type*} [category C] [limits.has_products C] (X Y : SheafedSpace C) (H : X.to_PresheafedSpace ≅ Y.to_PresheafedSpace) : X ≅ Y :=
 { hom := H.hom,
   inv := H.inv,
...
(Top_component hm f_deg) (Sheaf_component 𝒜 f_deg hm)
