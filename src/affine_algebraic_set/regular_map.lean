...
∃ Φ : k[n] →ₐ[k] k[V], ∀ (v : (V : subset_of 𝔸ᵐ)) (G : k[n]), eval (φ v : n → k) G = (Φ G) v
...
begin
  ...
  -- unfold is_morphism2,
  -- -- eval₂ gives the map
  -- let Φ.to_fun : mv_polynomial n k → regular_fun V := eval₂ _ F,
...
instance : has_coe_to_fun (V →ᵣ W) _ :=
{ coe := morphism.to_fun }
