...
def is_regular2 (f : (V : subset_of 𝔸ⁿ) → k) : Prop :=
  ∃ F : k[n], ∀ (x : 𝔸ⁿ) (hx : x ∈ V), eval x F = f ⟨x, hx⟩

def is_regular_fun (f : (V : subset_of 𝔸ⁿ) → k) : Prop :=
  ∃ F : k[n], ∀ (x : (V : subset_of 𝔸ⁿ)), eval (x : n → k) F = f x
...
(is_regular_fun' : is_regular_fun to_fun)
...
instance : has_coe_to_fun (k[V]) (λ _, _) :=
⟨regular_fun.to_fun⟩
...
lemma is_regular (f : k[V]): ∃ F : k[n],
   ∀ (x : (V : subset_of 𝔸ⁿ)), eval (x : n → k) F = f x := f.is_regular_fun'

variable (V)

noncomputable def mk' (V : affine_algebraic_set k n) (F : k[n]) : regular_fun V :=
{ to_fun := λ x, eval (x : n → k) F, -- i.e. F(x)
  is_regular_fun' := ⟨F, λ x, rfl⟩ }

variable {V}

def mk'.some_spec (F : k[n]) (x : (V : subset_of 𝔸ⁿ)) :
  eval (x : n → k) (classical.some (mk' V F).is_regular) = _ := classical.some_spec (mk' V F).is_regular x
...
{ to_fun := to_regular_fun.to_fun,
  ...
  map_mul' := begin
    intros f g,
    ext x,
    simp only [ring_hom.to_fun_eq_coe, map_mul],
  end,
  map_zero' := begin
    ext x,
    cases x with x hx,
    simp only [ring_hom.to_fun_eq_coe, map_zero],
  end,
  map_add' := begin
    intros f g,
    ext x,
    simp only [ring_hom.to_fun_eq_coe, map_add],
  end,
  commutes' := begin
    intro s,
    refl,
