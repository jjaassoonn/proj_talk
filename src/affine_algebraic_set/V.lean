...
lemma 𝕍_univ {k : Type*} [comm_ring k] [nontrivial k] {n : Type*} :
...
/-- If k ≠ 0 then 𝕍({1}) = ∅ -/
lemma 𝕍_one {k : Type*} [comm_ring k] [nontrivial k] {n : Type*} :
...
theorem 𝕍_mul {k : Type*} [comm_ring k] [nontrivial k]  [is_domain k] {n : Type*}
...
the radical of I -/
theorem 𝕍_radical' {k : Type*} [comm_ring k] [is_domain k] {n : Type*}
...
begin
  ...
  haveI : is_noetherian_ring (mv_polynomial n k) := sorry,
